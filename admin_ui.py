...
CC_WIZARD_STEPS = {
    "plan": [
        ...
        ("price_coins", "قیمت پلن را به الماس و فقط عددی بفرستید."),
...
class AdminPanelMixin:
    ...
    def cc_subscriptions_view(self) -> tuple[str, InlineKeyboardMarkup]:
        ...
        for plan in plans:
            ...
            lines.append(
                ...
                f"{int(plan['price_coins']):,} الماس — {state}"
            )
            rows.append(
                [
                    cc_button(
    ...
    def cc_plan_view(self, plan_id: int) -> tuple[str, InlineKeyboardMarkup]:
        ...
        text = (
            ...
            f"قیمت: {int(plan['price_coins']):,} الماس\n"
    ...
    def cc_finance_view(self) -> tuple[str, InlineKeyboardMarkup]:
        ...
        text = (
            ...
            f"مجموع موجودی کاربران: {wallet_total:,} الماس\n"
    ...
    def cc_financial_report_view(self) -> tuple[str, InlineKeyboardMarkup]:
        ...
        text = (
            ...
            f"├ افزایش موجودی: {summary['credits']:,} الماس\n"
            f"├ کسر موجودی: {summary['debits']:,} الماس\n"
            f"├ هزینه روزانه سلف: {summary['daily_fees']:,} الماس\n"
            f"└ تمدید خودکار: {summary['renewals']:,} الماس"
        )
        return text, InlineKeyboardMarkup(
    ...
    async def cc_finish_wizard(
        ...
        if action == "balance":
            ...
            return (
                f"✅ موجودی کاربر {user_id} به {balance:,} الماس تغییر کرد.",
    ...
    async def receive_control_center_text(
        ...
        try:
            ...
            if action in CC_WIZARD_STEPS:
                ...
                result = (
                    self.user_detail_text(
                        user_id, f"✅ موجودی به {balance:,} الماس تغییر کرد."
                    ),
