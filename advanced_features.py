...
class AdvancedFeatureEngine:
    ...
    def normalize(value: str) -> str:
        ...
        # دستورات این بخش فقط فارسی و بدون نقطه هستند.
        if text.startswith((".", "/")):
            return ""
        return text
    ...
    async def handle_command(self, event) -> bool:
        ...
        # دستورهای نهایی فارسی و بدون نقطه
        if lower == "حالت آنلاین روشن":
            self.save_settings({"online_status": "on"})
            await self.safe_edit(event, "✅ حالت آنلاین روشن شد.")
            return True
        if lower == "حالت آنلاین خاموش":
            self.save_settings({"online_status": "off"})
            await self.safe_edit(event, "✅ حالت آنلاین خاموش شد.")
            return True

        if lower == "حالت تایپینگ روشن":
            self.save_settings({"typing_action": "on"})
            await self.safe_edit(event, "✅ حالت تایپینگ روشن شد.")
            return True
        if lower == "حالت تایپینگ خاموش":
            self.save_settings({"typing_action": "off"})
            await self.safe_edit(event, "✅ حالت تایپینگ خاموش شد.")
            return True
        m = re.fullmatch(r"تنظیم زمان تایپینگ\s+(\d+)", lower)
        if m:
            self.save_settings({"typing_duration": str(max(1, min(int(m.group(1)), 60)))})
            await self.safe_edit(event, "✅ زمان تایپینگ تنظیم شد.")
            return True

        if lower == "منشی عمومی روشن":
            self.save_settings({"secretary": "on"})
            await self.safe_edit(event, "✅ منشی عمومی روشن شد.")
            return True
        if lower == "منشی عمومی خاموش":
            self.save_settings({"secretary": "off"})
            await self.safe_edit(event, "✅ منشی عمومی خاموش شد.")
            return True
        m = re.fullmatch(r"تنظیم منشی عمومی\s+([\s\S]+)", raw, re.I)
        if m:
            self.save_settings({"secretary_fallback_text": m.group(1).strip()[:3500]})
            await self.safe_edit(event, "✅ متن منشی عمومی ثبت شد.")
            return True

        if lower == "منشی روشن":
            self.save_settings({"offline_reply_enabled": "on"})
            await self.safe_edit(event, "✅ منشی آفلاین روشن شد.")
            return True
        if lower == "منشی خاموش":
            self.save_settings({"offline_reply_enabled": "off"})
            await self.safe_edit(event, "✅ منشی آفلاین خاموش شد.")
            return True
        m = re.fullmatch(r"تنظیم متن منشی\s+([\s\S]+)", raw, re.I)
        if m:
            self.save_settings({"offline_reply_text": m.group(1).strip()[:3500]})
            await self.safe_edit(event, "✅ متن منشی ثبت شد.")
            return True

        for prefix, key in (
            ("ساعت اسم", "timename"), ("ساعت بیو", "timebio"),
        ):
            if lower == f"{prefix} روشن":
                self.save_settings({key: "on"})
                await self.safe_edit(event, f"✅ {prefix} روشن شد.")
                return True
            if lower == f"{prefix} خاموش":
                self.save_settings({key: "off"})
                await self.safe_edit(event, f"✅ {prefix} خاموش شد.")
                return True
        m = re.fullmatch(r"تنظیم فونت ساعت اسم\s+([1-9]|10)", lower)
        if m:
            self.save_settings({"timename_font": m.group(1)})
            await self.safe_edit(event, "✅ فونت ساعت اسم تنظیم شد.")
            return True
        m = re.fullmatch(r"تنظیم فونت ساعت بیو\s+([1-9]|10)", lower)
        if m:
            self.save_settings({"timebio_font": m.group(1)})
            await self.safe_edit(event, "✅ فونت ساعت بیو تنظیم شد.")
            return True

        if lower == "ذخیره عکس زمان‌دار روشن":
            self.save_settings({"save_timed_photos": "on"})
            await self.safe_edit(event, "✅ ذخیره عکس زمان‌دار روشن شد.")
            return True
        if lower == "ذخیره عکس زمان‌دار خاموش":
            self.save_settings({"save_timed_photos": "off"})
            await self.safe_edit(event, "✅ ذخیره عکس زمان‌دار خاموش شد.")
            return True

        if lower == "امضا پیام روشن":
            self.save_settings({"outgoing_signature_enabled": "on"})
            await self.safe_edit(event, "✅ امضا پیام روشن شد.")
            return True
        if lower == "امضا پیام خاموش":
            self.save_settings({"outgoing_signature_enabled": "off"})
            await self.safe_edit(event, "✅ امضا پیام خاموش شد.")
            return True
        m = re.fullmatch(r"تنظیم متن امضا\s+([\s\S]+)", raw, re.I)
        if m:
            self.save_settings({"outgoing_signature_text": m.group(1).strip()[:300]})
            await self.safe_edit(event, "✅ متن امضا ثبت شد.")
            return True

        m = re.fullmatch(r"عضویت اجباری\s+(روشن|خاموش)", lower)
        if m:
            state = self.on_off(m.group(1))
            self.save_settings({"force_join_private": state})
            await self.safe_edit(event, f"✅ عضویت اجباری {'روشن' if state == 'on' else 'خاموش'} شد.")
            return True
        m = re.fullmatch(r"تنظیم کانال عضویت اجباری\s+(.+)", raw, re.I)
        if m:
            ...
            if value.lstrip("-").isdigit():
                username = ""
            url = value if value.startswith(("http://", "https://")) else f"https://t.me/{username}" if username else ""
            set_app_settings(self.users_db, {"force_join_chat_id": chat_id, "force_join_username": username, "force_join_url": url})
            await self.safe_edit(event, "✅ کانال عضویت اجباری ثبت شد.")
            return True

        for scope, key in (("پیوی", "auto_read_private"), ("گروه", "auto_read_groups"), ("کانال", "auto_read_channels")):
            if lower == f"سین خودکار {scope} روشن":
                self.save_settings({key: "on"})
                await self.safe_edit(event, f"✅ سین خودکار {scope} روشن شد.")
                return True
            if lower == f"سین خودکار {scope} خاموش":
                self.save_settings({key: "off"})
                await self.safe_edit(event, f"✅ سین خودکار {scope} خاموش شد.")
                return True

        m = re.fullmatch(r"ری اکت خودکار همه پیام ها\s+(.+)", raw, re.I)
        if m:
            value = m.group(1).strip()
            if value == "خاموش":
                self.save_settings({"auto_reaction": "off"})
                await self.safe_edit(event, "✅ ری اکت خودکار همه پیام ها خاموش شد.")
            else:
                self.save_settings({"auto_reaction": "on", "auto_reaction_emoji": value[:16]})
                await self.safe_edit(event, "✅ ری اکت خودکار همه پیام ها ثبت شد.")
            return True
        m = re.fullmatch(r"ری اکت خودکار\s+(.+)", raw, re.I)
        if m and not m.group(1).strip().startswith("همه پیام ها"):
            if not getattr(event, "is_reply", False):
            ...
            if not reply or not getattr(reply, "sender_id", None):
                ...
                return True
            try:
                targets = json.loads(self.settings().get("auto_reaction_targets", "{}") or "{}")
            except Exception:
                targets = {}
            targets[str(int(reply.sender_id))] = m.group(1).strip()[:16]
            self.save_settings({"auto_reaction_targets": json.dumps(targets, ensure_ascii=False)})
            await self.safe_edit(event, "✅ ری اکت خودکار برای این شخص ثبت شد.")
            return True
        m = re.fullmatch(r"لغو ری اکت خودکار(?:\s+(.+))?", raw, re.I)
        if m:
            if not getattr(event, "is_reply", False):
                await self.safe_edit(event, "❌ برای لغو، روی پیام همان شخص ریپلای کنید.")
                return True
            reply = await self.replied_message(event)
            try:
                targets = json.loads(self.settings().get("auto_reaction_targets", "{}") or "{}")
                ...
                targets = {}
            targets.pop(str(int(reply.sender_id)), None)
            self.save_settings({"auto_reaction_targets": json.dumps(targets, ensure_ascii=False)})
            await self.safe_edit(event, "✅ ری اکت خودکار شخص لغو شد.")
            return True

        m = re.fullmatch(r"اکشن\s+(تایپ|بازی|ویس|صدا|ویدیو|عکس|فایل|موقعیت|استیکر)\s+روشن", lower)
        if m:
            self.save_settings({"action_mode": ACTION_ALIASES.get(m.group(1), m.group(1))})
            await self.safe_edit(event, f"✅ اکشن {m.group(1)} روشن شد.")
            return True
        if lower == "اکشن خاموش":
            self.save_settings({"action_mode": "off"})
            await self.safe_edit(event, "✅ اکشن خاموش شد.")
            return True

        lower = raw.lower()

        if lower == "تنظیم پروفایل":
            await self.copy_profile(event)
            return True

        if lower in {"آن بن", "ان بن"}:
            if not getattr(event, "is_group", False) or not getattr(event, "is_reply", False):
            ...
            try:
                ...
                await self.safe_edit(event, "✅ بن کاربر برداشته شد.")
            except Exception as exc:
                await self.safe_edit(event, f"❌ رفع بن انجام نشد: {type(exc).__name__}")
            return True
        ...
        if lower == "بن":
            ...
            try:
                ...
                await self.safe_edit(event, "✅ کاربر بن شد.")
            except Exception as exc:
                await self.safe_edit(event, f"❌ بن انجام نشد: {type(exc).__name__}")
            return True
        ...
        if lower in {"گزارش مدیران"}:
            await self.report_to_admins(event)
            return True

        match = re.fullmatch(
            r"خوش آمد گویی\s+(روشن|خاموش)",
            lower,
        )
        if match:
            state = self.on_off(match.group(1))
        ...
        match = re.fullmatch(
            r"تنظیم متن خوش آمد گویی\s+([\s\S]+)",
            raw,
            re.IGNORECASE,
        )
        if match:
        ...
        if lower in {"آمار حساب"}:
            await self.show_account_stats(event)
            return True

        match = re.fullmatch(r"کیوآر\s+([\s\S]+)", raw, re.I)
        if match:
            await self.create_qr(event, match.group(1).strip())
            return True

        match = re.fullmatch(
            r"ساعت عکس\s+(روشن|خاموش)",
            lower,
        )
        if match:
            state = self.on_off(match.group(1))
