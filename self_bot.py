...
✅ **اکانت با موفقیت غیرفعال شد**
    ...
    async def generate_help_text(self):
        ...
        return (
            ...
            "همه دستورهای فارسی بدون نقطه هستند.\n"
    ...
    async def secretary_handler(self, event):
        ...
        await event.edit(secretary_text)
    ...
    async def register_settings_handlers(self):
        ...
        async def settings_handler(event):
            try:
                ...
                command_key = {
                    ...
                    "فوروارد خودکار": "autoforward",
                    "زمان نام": "timename",
                    "زمان بیو": "timebio",
                    "ذخیره عکس": "save",
                    "زمانبندی": "schedule",
                }[command]
                value = {"روشن": "on", "خاموش": "off"}[value]
                command = command_key
                setting_key = {
                ...
                command_names = {
                    ...
                    "timename": "زمان در نام خانوادگی",
                    "timebio": "زمان در بیوگرافی",
                    "save": "ذخیره عکس زمان‌دار",
                    "schedule": "ارسال زمان‌بندی‌شده",
                }
                state_word = "فعال شد" if value == "on" else "غیرفعال شد"
                response_msg = (
                    f"{command_names.get(command, command)} {state_word} ✓"
                    f"{extra_line}"
                )
        ...
        @self.client.on(events.NewMessage(pattern=r'^تایپینگ (\d+)$'))
        async def typing_duration_handler(event):
            try:
                ...
                await event.edit(f"مدت زمان تایپینگ روی {duration} ثانیه تنظیم شد ✓")
        ...
        async def persian_stop_schedule_handler(event):
            try:
                ...
                self.put_data(settings)
                await event.edit("ارسال زمان‌بندی‌شده غیرفعال شد ✓")
            except Exception as exc:
                print(
        ...
        @self.client.on(events.NewMessage(pattern=r'^زمانبندی هر (\d+)$'))
        ...
        @self.client.on(events.NewMessage(pattern=r'^زمانبندی مقصد (.+)$'))
        ...
        @self.client.on(events.NewMessage(pattern=r'^زمانبندی متن (.+)$'))
        async def scheduled_text_handler(event):
            try:
                ...
                settings["scheduled_message_text"] = message_text
                self.put_data(settings)
                await event.edit("متن پیام زمان‌بندی‌شده ذخیره شد ✓")
            except Exception as exc:
                print(
        ...
        @self.client.on(events.NewMessage(pattern=r'^فونت ([1-9]|10)$'))
        async def font_handler(event):
            try:
                ...
                await event.edit(f"فونت زمان روی شماره {font_num} تغییر کرد ✓")
                
            except Exception as e:
                print(f"خطا در تغییر فونت برای {self.phone}: {e}")
        
        @self.client.on(events.NewMessage(pattern=r'^(کراش افزودن|کراش حذف|دشمن افزودن|دشمن حذف) (.*)$'))
        async def user_management_handler(event):
            try:
                ...
                command = {
                    "کراش افزودن": "addcrash",
                    "کراش حذف": "delcrash",
                    "دشمن افزودن": "addenemy",
                    "دشمن حذف": "delenemy",
                }[command]
        ...
        @self.client.on(events.NewMessage(pattern=r'^پاکسازی (\d+)$'))
        ...
        @self.client.on(events.NewMessage(pattern=r'^افزودن پاسخ (.+)\|(.+)$'))
    ...
    async def check_expiration(self):
        """بررسی انقضای اکانت"""
        # هنگام فعال‌سازی، main_bot ابتدا self_enabled را 0 می‌گذارد و سلف را
        # اجرا می‌کند، و فقط *بعد از* تأیید آماده‌بودن پردازش، آن را 1 می‌کند.
        # اگر این تسک بلافاصله چک کند، ممکن است دقیقاً همان لحظه‌ی self_enabled=0
        # را ببیند و اکانت تازه‌فعال‌شده را «منقضی» تشخیص بدهد و قطع کند. برای
        # جلوگیری از این رقابت زمانی، اولین بررسی را کمی به تأخیر می‌اندازیم.
        while self.is_running and not self.shutdown_requested:
            await asyncio.sleep(60)
