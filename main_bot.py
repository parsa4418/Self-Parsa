...
class TelegramAuthBot(AdminPanelMixin):
    ...
    def save_activated_user(self, user_id, phone, process, session_file):
        ...
        raw_pid = getattr(process, "pid", process)
        if raw_pid is None:
            runtime = self.active_selfbots.get(int(user_id)) or {}
            raw_pid = runtime.get("pid")
        if raw_pid is None:
            record = self.get_selfbot_record(int(user_id))
            raw_pid = record["self_pid"] if record else None
        if raw_pid is None:
            raise RuntimeError("فرایند سلف اجرا شده است اما PID آن قابل تشخیص نیست.")
    ...
    def record_selfbot_failure(self, user_id: int, detail: str, permanent: bool = False) -> None:
        """ثبت خطای اجرای سلف بدون وابستگی به متد حذف‌شدهٔ قبلی."""
        user_id = int(user_id)
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        try:
            record = self.get_selfbot_record(user_id)
            failures = int((record["self_consecutive_failures"] if record else 0) or 0) + 1
            status = "failed" if permanent else "restart_pending"
            self.update_selfbot_runtime(
                user_id,
                self_pid=None,
                self_status=status,
                self_last_error=str(detail or "خطای نامشخص"),
                self_last_stopped_at=now,
                self_consecutive_failures=failures,
                self_next_restart_at=None,
            )
        except Exception:
            logging.exception("Could not record selfbot failure for %s", user_id)
            ...
            if not success:
                ...
                try:
                    ...
                    self.save_activated_user(
                        user_id, phone_number, process_info.get('pid'),
