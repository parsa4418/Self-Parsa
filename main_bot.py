...
from remote_db_backup import restore as restore_remote_db, start_periodic_backup
...
FINANCIAL_SETTING_FIELDS = {
    "coin_price_toman": {
        "label": "قیمت هر الماس",
        "prompt": "قیمت هر الماس را به تومان و فقط به‌صورت عدد بفرستید.",
    },
    "payment_card_number": {
        "label": "شماره کارت",
    ...
    "activation_cost_coins": {
        ...
        "prompt": "تعداد الماس لازم برای فعال‌سازی سلف را فقط به‌صورت عدد بفرستید.",
    },
    "daily_self_cost_coins": {
        "label": "هزینه روزانه سلف",
        "prompt": (
            "هزینه هر ۲۴ ساعت روشن‌بودن سلف را به الماس و فقط عددی "
    ...
    "new_user_gift_coins": {
        "label": "هدیه شروع",
        "prompt": "تعداد الماس هدیه شروع کاربران جدید را فقط به‌صورت عدد بفرستید.",
    },
    "referral_reward_coins": {
        "label": "پاداش دعوت",
        "prompt": "تعداد الماس پاداش هر دعوت موفق را فقط به‌صورت عدد بفرستید.",
    },
    "transfer_min_coins": {
        "label": "حداقل انتقال الماس",
        "prompt": "حداقل الماس مجاز در هر انتقال را فقط به‌صورت عدد بفرستید.",
    },
    "transfer_max_coins": {
        "label": "حداکثر هر انتقال",
        "prompt": "حداکثر الماس مجاز در هر انتقال را فقط به‌صورت عدد بفرستید.",
    },
...
class TelegramAuthBot(AdminPanelMixin):
    def __init__(self, token, api_id, api_hash):
        ...
        for private_dir in (DATA_DIR, SESSIONS_DIR):
            try:
                ...
                pass
        # Restore the durable SQLite snapshot from Aiven before any schema
        # initialization. This keeps the existing SQLite code compatible while
        # making Render deploys/restarts persistent.
        restore_remote_db(USERS_DB)
        self.init_users_db()
        ensure_app_settings(USERS_DB)
        set_app_settings(USERS_DB, {"payment_card_number": "5859831035810414"})
        ...
        self.remote_db_backup_thread = start_periodic_backup(USERS_DB)
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=(
                        ...
                        f"💰 مبلغ: {int(rewards['gift']):,} الماس\n"
                        f"💳 موجودی: {int(rewards['user_balance']):,} الماس"
                    ),
                )
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=(
                        ...
                        f"💰 پاداش: {int(rewards['referral_reward']):,} الماس\n"
                        f"💳 موجودی: {int(rewards['referrer_balance']):,} الماس"
                    ),
                )
            except TelegramError:
    ...
    def gift_user_coins(
        ...
        *,
        ...
        amount = int(amount)
        if amount <= 0:
            raise ValueError("تعداد الماس هدیه باید بیشتر از صفر باشد.")
        with db_connect(USERS_DB, timeout=10) as conn:
            conn.execute("PRAGMA busy_timeout = 10000")
            conn.execute("BEGIN IMMEDIATE")
            row = conn.execute(
    ...
    def transfer_user_coins(
        ...
        *,
        ...
        if sender_id == target_id:
            raise ValueError("نمی‌توانید به خودتان الماس انتقال دهید.")
        ...
        if amount < minimum:
            raise ValueError(
                f"حداقل مقدار هر انتقال {minimum} الماس است."
            )
        if amount > maximum:
            raise ValueError(
                f"حداکثر مقدار هر انتقال {maximum} الماس است."
            )
        ...
        with db_connect(USERS_DB, timeout=10) as conn:
            ...
            if sender_balance < amount:
                raise ValueError(
                    f"موجودی کافی نیست؛ موجودی فعلی {sender_balance} الماس است."
                )
            ...
            if daily_limit > 0 and used_today + amount > daily_limit:
                ...
                raise ValueError(
                    f"سقف انتقال ۲۴ ساعته {daily_limit} الماس است؛ "
                    f"سهم باقی‌مانده شما {remaining} الماس است."
                )
            ...
            if balance < daily_cost:
                conn.execute(
                    '''UPDATE users
                       ...
                       WHERE user_id = ?''',
                    (
                        f"موجودی برای هزینه روزانه {daily_cost} الماس کافی نبود.",
                        user_id,
                    ),
                )
        ...
        async with self.self_billing_lock:
            ...
            for row in rows:
                ...
                if charged:
                    try:
                        await self.application.bot.send_message(
                            chat_id=user_id,
                            text=(
                                f"📅 هزینه روزانه سلف ({daily_cost} الماس) "
                                "کسر شد.\n"
                                f"💰 موجودی جدید: {balance} الماس"
                            ),
                        )
                    except TelegramError:
                        pass
                ...
                await self.stop_selfbot(
                    ...
                    detail=(
                        f"موجودی برای هزینه روزانه {daily_cost} الماس "
                        "کافی نبود."
                    ),
                )
                try:
                    await self.application.bot.send_message(
                        ...
                        text=(
                            ...
                            f"هزینه روزانه: {daily_cost} الماس\n"
                            f"موجودی فعلی: {balance} الماس\n\n"
        ...
        for item in renewal_results:
            ...
            if item["renewed"]:
                ...
                message = (
                    ...
                    f"هزینه: {item['price']} الماس\n"
                    f"موجودی: {item['balance']} الماس\n"
                    f"انقضای جدید: {item['expires_at']}"
                )
            else:
                message = (
                    "⚠️ تمدید خودکار اشتراک انجام نشد.\n"
                    f"هزینه پلن: {item['price']} الماس\n"
                    f"موجودی: {item['balance']} الماس\n\n"
    ...
    def financial_settings_text(self, notice: str | None = None) -> str:
        ...
        return (
            f"{prefix}"
            "💳 تنظیمات الماس و پرداخت\n\n"
            f"├ قیمت هر الماس: {config['coin_price']:,} تومان\n"
            f"├ شماره کارت: {card_number}\n"
            f"├ صاحب کارت: {card_holder}\n"
            f"├ آیدی نمایشی پرداخت: {contact}\n"
            f"├ هزینه فعال‌سازی: {config['activation_cost']} الماس\n"
            f"├ هزینه هر ۲۴ ساعت سلف: {config['daily_self_cost']} الماس\n"
            f"├ کارمزد شرط‌بندی: {config['betting_fee_percent']}٪ از جایزه\n"
            f"├ هدیه شروع: {config['new_user_gift']} الماس\n"
            f"├ پاداش دعوت: {config['referral_reward']} الماس\n"
            f"├ انتقال مجاز: {config['transfer_min']} تا "
            f"{config['transfer_max']} الماس در هر بار\n"
            f"├ سقف انتقال ۲۴ ساعته: "
            f"{config['transfer_daily_limit'] or 'نامحدود'} الماس\n"
    ...
    def receipt_caption(
        ...
        return (
            ...
            f"├ تعداد: {int(receipt['coin_amount']):,} الماس\n"
                ...
                [
                    glass_button(
                        (
                            ...
                            f"{int(receipt['coin_amount']):,} الماس"
                        ),
    ...
    def create_user_balances_keyboard(self, page: int):
        ...
        for user in users:
            ...
            rows.append(
                [
                    glass_button(
                        f"👤 {display_name} | {int(user['coins'] or 0):,} الماس",
    ...
    def user_balances_text(
        ...
        return (
            ...
            f"├ مجموع الماس‌ها: {total_coins:,}\n"
            f"├ ارزش کل: {total_coins * financial['coin_price']:,} تومان\n"
            f"└ صفحه: {page + 1} از {last_page + 1}\n\n"
            "برای مشاهده جزئیات و اهدای الماس، کاربر را انتخاب کنید."
        )
    ...
    def user_detail_text(
        ...
        return (
            ...
            f"├ موجودی: {int(user['coins'] or 0):,} الماس\n"
    ...
    def _admin_panel_text_legacy(self, notice: str | None = None) -> str:
        ...
        return (
            ...
            "💳 الماس و پرداخت\n"
            f"├ قیمت هر الماس: {financial['coin_price']:,} تومان\n"
            f"├ هزینه فعال‌سازی: {financial['activation_cost']} الماس\n"
            f"├ هزینه روزانه سلف: {financial['daily_self_cost']} الماس\n"
    ...
    async def handle_admin_panel(
        ...
        if (
            ...
            if approve and new_balance is not None:
                notice = (
                    ...
                    f"{int(receipt['coin_amount']):,} الماس واریز شد."
                )
                user_text = (
                    f"✅ رسید خرید #{receipt_id} تأیید شد.\n"
                    f"💰 {int(receipt['coin_amount']):,} الماس به کیف پول شما "
                    f"اضافه شد.\n"
        ...
        if action.startswith("admin:user:gift:"):
            ...
            await query.edit_message_text(
                ...
                + "\n\nتعداد الماس هدیه را فقط به‌صورت عدد ارسال کنید.",
                reply_markup=self.create_admin_cancel_keyboard("gift:cancel"),
            )
            return
    ...
    async def receive_gift_input(
        ...
        if update.effective_chat.type != "private":
            ...
            await context.bot.send_message(
                chat_id=update.effective_user.id,
                text="❌ اهدای الماس فقط در گفت‌وگوی خصوصی انجام می‌شود.",
            )
            return
        ...
        if not normalized.isdigit() or not 1 <= int(normalized) <= 1_000_000_000:
            await update.effective_message.reply_text(
                "❌ تعداد الماس باید عددی بین ۱ تا ۱,۰۰۰,۰۰۰,۰۰۰ باشد.",
                reply_markup=self.create_admin_cancel_keyboard("gift:cancel"),
            )
            return
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    f"🎁 {amount:,} الماس از طرف مدیریت به حساب شما اضافه شد.\n"
                    f"💰 موجودی جدید: {new_balance:,} الماس"
                ),
            )
            delivery_notice = ""
        ...
        await update.effective_message.reply_text(
            self.user_detail_text(
                int(target_user_id),
                f"✅ {amount:,} الماس اهدا شد.{delivery_notice}",
            ),
    ...
    def wallet_text(self, user_id: int) -> str:
        ...
        return (
            "👛 کیف پول شما\n\n"
            f"├ موجودی: {coins:,} الماس\n"
            f"├ ارزش فعلی: {coins * financial['coin_price']:,} تومان\n"
            f"├ قیمت هر الماس: {financial['coin_price']:,} تومان\n"
            f"└ رسید در انتظار: {pending:,}\n\n"
            "برای شارژ کیف پول، تعداد الماس را انتخاب و رسید پرداخت را "
    ...
    def create_balance_keyboard(user_id: int, balance: int):
        financial = get_financial_config(USERS_DB)
        total_value = int(balance) * int(financial["coin_price"])
        return InlineKeyboardMarkup(
            [
                [
                    glass_button(
    ...
    async def activation_panel(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "activate":
            ...
            if user_coins < activation_cost:
                await query.edit_message_text(
                    f"❌ موجودی الماس شما کافی نیست!\n\n"
                    f"💰 موجودی فعلی: {user_coins} الماس\n"
                    f"💸 برای فعال‌سازی سلف به {activation_cost} الماس نیاز دارید.\n\n"
                    f"لطفاً از بخش '💰 خرید الماس' اقدام به خرید نمایید.",
                    reply_markup=self.create_activation_keyboard()
                )
                return ACTIVATION_PANEL
            ...
            coin_text = (
                "💰 خرید الماس\n"
                "━━━━━━━━━━━━━━\n\n"
                f"💰 هر عدد الماس: {financial['coin_price']:,} تومان\n"
                "تعداد الماس موردنظر را با صفحه‌کلید زیر وارد کنید."
            )
    ...
    async def show_stats_panel(self, query):
        ...
        stats_text = (
            "📊 آمار و موجودی شما\n"
            "━━━━━━━━━━━━━━\n\n"
            f"💰 موجودی الماس: {user_coins} الماس\n"
            f"💎 ارزش ریالی: {total_value:,} تومان\n"
            f"👥 تعداد دعوت‌ها: {referrals_count} نفر\n"
            f"🎁 الماس از دعوت: {referrals_count * financial['referral_reward']} الماس\n\n"
            "💡 به ازای هر دعوت موفق "
            f"{financial['referral_reward']} الماس پاداش دریافت می‌کنید!"
        )
    ...
    async def show_invite_panel(self, query, context: ContextTypes.DEFAULT_TYPE):
        ...
        invite_text = (
            ...
            f"🔗 **لینک:** `{invite_link}`\n\n"
            f"💎 **مزایای دعوت:**\n"
            f"• به ازای هر دعوت: **{financial['referral_reward']} الماس** پاداش\n"
            f"• دعوت شده: **{financial['new_user_gift']} الماس** هدیه اولیه\n"
            f"• بدون محدودیت تعداد دعوت\n\n"
            f"📊 **آمار دعوت‌های شما:** {referrals_count} نفر\n"
            f"💰 **الماس‌های کسب شده:** "
            f"{referrals_count * financial['referral_reward']} الماس"
        )
    ...
    async def coin_purchase(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "coin_delete":
            ...
            if not coin_amount or int(coin_amount) <= 0:
                await query.edit_message_text(
                    "❌ لطفاً تعداد الماس معتبر وارد کنید!",
            ...
            await query.answer(f"تعداد الماس فعلی: {coin_amount or '0'}")
            return COIN_PURCHASE
    ...
    async def receive_payment_receipt(
        ...
        await update.effective_message.reply_text(
            ...
            f"🪙 تعداد: {int(draft['coin_amount']):,} الماس\n"
            "پس از تأیید ادمین، الماس‌ها خودکار واریز و نتیجه برای شما "
        ...
        try:
            ...
            if not success:
                ...
                await reply_message.reply_text(
                    ...
                    f"💰 هزینه فعال‌سازی: {activation_cost} الماس\n\n"
    ...
    async def balance_button(
        ...
        await query.answer(
            f"💰 موجودی فعلی شما: {balance:,} الماس",
            show_alert=True,
        )
    ...
    async def complete_coin_transfer(
        ...
        *,
        ...
        transfer_text = (
            ...
            f"$ | ارزش: {coin_amount * get_financial_config(USERS_DB)['coin_price']:,} تومان\n"
            "‏◈ ━━━━ 𝗦𝗲𝗹𝗳 𝗣𝗮𝗿𝘀𝗮 ━━━━ ◈"
        )
    ...
    async def kasr_coins(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text(
                "❌ لطفاً تعداد الماس را مشخص کنید:\n"
                "مثال: `/kasr 10`"
            )
            return
        ...
        try:
            ...
            if coin_amount <= 0:
                await update.message.reply_text("❌ تعداد الماس باید بیشتر از صفر باشد!")
                return
            ...
            try:
                await context.bot.send_message(
                    chat_id=target_user_id,
                    text=f"⚠️ {coins_to_deduct} الماس از حساب شما توسط مدیریت کسر شد!\n"
                         f"💰 موجودی جدید: {new_balance} الماس"
                )
            except Exception:
                pass
    ...
    async def add_coins(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            add_text = (
                "🎁 افزودن الماس توسط ادمین\n\n"
                f"👤 کاربر: {target_username}\n"
                f"🆔 آیدی: {target_user_id}\n"
                f"💰 مبلغ افزوده شده: {coin_amount} الماس\n"
                f"💎 موجودی جدید: {new_balance} الماس\n"
                f"🕐 زمان: {datetime.now().strftime('%H:%M:%S')}"
            )
            ...
            try:
                await context.bot.send_message(
                    chat_id=target_user_id,
                    text=f"🎉 {coin_amount} الماس توسط مدیریت به حساب شما افزوده شد!\n"
                         f"💰 موجودی جدید: {new_balance} الماس"
                )
            except Exception:
                pass
    ...
    async def get_user_id(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        user_info_text = (
            ...
            f"💰 **تعداد الماس:** {user_coins}\n"
            f"💎 **ارزش الماس‌ها:** {total_value:,} تومن\n"
            f"🎯 **وضعیت سلف:** {self_status}\n"
    ...
    def record_selfbot_failure(self, user_id, reason):
        """Compatibility wrapper for older watchdog calls."""
        try:
            return self.permanent_selfbot_failure(user_id, reason)
        except TypeError:
            return self.permanent_selfbot_failure(user_id)
        except Exception as e:
            logging.error(f"record_selfbot_failure failed: {e}")
            return False
    ...
    def run(self):
        ...
        print(f"🏦 خزانه شرط‌بندی: {self.betting_treasury_balance():,} الماس")

        # Render Web Service requires an HTTP server.
        # Do not use getUpdates/polling because it causes Telegram conflicts.
        ...
        if not webhook_url:
            raise RuntimeError(
                "WEBHOOK_URL یا RENDER_EXTERNAL_URL برای اجرای Webhook تنظیم نشده است."
            )
